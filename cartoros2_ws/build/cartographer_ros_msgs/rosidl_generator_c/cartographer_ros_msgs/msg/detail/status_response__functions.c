  ...
  cartographer_ros_msgs__msg__StatusResponse__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    cartographer_ros_msgs__msg__StatusResponse * data =
      (cartographer_ros_msgs__msg__StatusResponse *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cartographer_ros_msgs__msg__StatusResponse__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          cartographer_ros_msgs__msg__StatusResponse__fini(&data[i]);
        }
        free(data);
        return false;
      }
