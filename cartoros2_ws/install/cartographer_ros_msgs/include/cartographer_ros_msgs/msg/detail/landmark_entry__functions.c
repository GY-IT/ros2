  ...
  cartographer_ros_msgs__msg__LandmarkEntry__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    cartographer_ros_msgs__msg__LandmarkEntry * data =
      (cartographer_ros_msgs__msg__LandmarkEntry *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!cartographer_ros_msgs__msg__LandmarkEntry__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          cartographer_ros_msgs__msg__LandmarkEntry__fini(&data[i]);
        }
        free(data);
        return false;
      }
